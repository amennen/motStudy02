...
% end
data_dir = [documents_path 'BehavioralData'];
dicom_dir = [documents_path 'data' filesep SUBJ_NAME]; %where all the dicom information is FOR THAT SUBJECT
if SESSION >= MOT{1}
    runNum = SESSION - MOT{1} + 1;
    classOutputDir = [dicom_dir filesep 'motRun' num2str(runNum) '/classOutput/'];
end
...
switch SESSION
    ...
    case {RECALL_PRACTICE,RECALL1,RECALL2}
        ...
        stim.instruct2 = ['After you have had several seconds to form an image, we will ask you how detailed it was. This rating is not a "test": ' ...
            'we want to understand your real experience, even if no image formed when you felt you SHOULD have had one. Similarly, you should not ' ...
            'adjust your rating based on how sure you are it''s the correct scene. Please make your rating based only on how much scene detail comes to you, ' ...
            'ignoring other mental imagery (e.g., you pictured a beach). You will use the provided rating scale to indicate details, using letters Z-Y on the keyboard.'...
            '\n\n---- Press ' PROGRESS_TEXT ' once you understand these instructions,\n then press it again when you are done viewing the rating scale ---'];
        ...
        if SESSION == RECALL_PRACTICE
            ...
            displayText(mainWindow,['MENTAL PICTURES TASK - PRACTICE\n\nThis is a memory test with some differences ' ...
                'from earlier: you will get only one try per item, there is no feedback, and you will be recorded instead of pushing keys.' ...
            ...
            condmap = makeMap({'realtime','omit'});
            if SESSION ==RECALL1
                displayText(mainWindow,['The experiment will now shift to using ONLY the stimuli that you studied yesterday, both ' ...
...
%         else
  ...
  %      end
        ...
        stim.instruct1 = [stim.header '\n\nWe will now do a "multi-tasking" twist: we would like you to ' ...
            ...
            'Though you will be tracking dots that are moving around the screen, is VITAL that you keep your eyes at the center fixation dot for the entire '...
            'time that you are tracking dot motion!!! When the ''question'' dot turns white, you '...
            'can then take your eyes off the center and indicate whether the dot was originally a target (PINKY) or not (THUMB). ' ...
            instruct_continue];
        stim.instruct2 = ['While all this happens, we want you to "multi-task" by visualizing the scene ' ...
            'named by the word in the middle. Every few seconds, the central dot will turn red: when it does, you should ' ...
            'indicate your CURRENT visualization detail using the rating scale provided. It''s possible the clarity of your ' ...
            'mental picture will change throughout the trial and/or be fuzzier than if you were not tracking dots. ' ...
            'As your rating should reflect your CURRENT rather than best possible image, this would be reflected ' ...
            'in your responses. The central dot will turn red four times in each trial. Your visualization response '...
            'will only count while the central dot is red. \n\nRemember that mental visualizing is only your second priority: if you lose track ' ...
            'of the dots for even a second, you will get the trial wrong, and we will have to throw out the trial; ' ...
            'and we need as many trials as possible. Getting the white dot correct is your most important task (but make sure '...
            'this is not at the expense of moving your eyes away from the center fixation dot; that would be cheating)! ' ...
            'Then, "squeeze in" visualizing when it''s possible. \n\n---- Press ' PROGRESS_TEXT ' once you understand these instructions,\n '...
            'then press it again when you are done viewing the rating scale ---'];
        stim.instruct_summary = [stim.header '\n\nTo summarize this task: you will keep track of target dots moving around the screen while keeping your ' ...
            ...
            'different trials. Visualizing the image may be easier when slower, or harder when dots are faster. Just try to do your best. '...
        ...
        stim.fMRI_refresher = ['MULTI-TASKING-- fMRI PRACTICE\n\nDot tracking today will be similar to last time, with two ' ...
            ...
            'Dot-tracking may be easier when slower, such that you can shift some effort to scene visualization in those trials.\n\n' ...
        ...
        for n=1:length(stim.cond)
                ...
                if realtime %just saying it's one of the MOT sessions
                    if TRcounter >= 4 %the first time we're looking is during TR 4
                        ...
                        if scanNum %if we should be looking for a file 
                            if ~mod(stim.frame_counter(n),3) && isnan(rtData.classOutputFileLoad(fileTR)) % look every 3 frames
                                ...
                                if (GetSecs < timing.plannedOnsets.tClassOutputFileTimeout(fileTR)) %don't need a min time because we're waiting for TRcounter to be 4
                                    if showFiles
                                        rtData.fileList{thisTR} = ls(classOutputDir) %saved for at the TR we're literally on, what are the available files
                                    else
                                        rtData.fileList{thisTR} = ls(classOutputDit);
                                    end
            ...
            if SESSION == MOT_PRACTICE && (stim.trial <= 3)
                ...
                displayText(mainWindow,['Also, you responded to ' num2str(round(100*sum(~isnan(train.resp))/100)) ' out of ' ...
    ...
    case SCAN_PREP
        ...
        displayText(mainWindow,['During the scan today, it is crucial that you keep your head still. Even a tiny head movement, e.g., caused by stretching your legs, will blur ' ...
            ...
            '-- please press the index finger button to continue --'],minimumDisplay,'center',COLORS.MAINFONTCOLOR,WRAPCHARS);
        waitForKeyboard(kbTrig_keycode,DEVICE);
        displayText(mainWindow,['As a final precaution, please try to avoid forming any ''loops'' with your body. That is, let your arms rest at your side, and do not cross ' ...
            'your legs. There is a chance if you do allow ''loops'' to form with your body, you could experience uncomfortable heating up as a result of the physics of the MRI ' ...
            'machine.\n\nAlso, please be advised that some participants experience a tingling sensation during the scan. Don''t be concerned if you experience this, it is ' ...
            'entirely safe and normal.\n\n-- please press the index finger button to continue --'],minimumDisplay,'center',COLORS.MAINFONTCOLOR,WRAPCHARS);
        waitForKeyboard(kbTrig_keycode,DEVICE);
       ...
       % runStart = timing.trig.wait;
        DrawFormattedText(mainWindow,'+','center','center',COLORS.MAINFONTCOLOR,WRAPCHARS);
        Screen('Flip', mainWindow)
        config.wait = 8; % we want this to be up for 8 seconds to collect sample TR's
        config.TR = 2;
