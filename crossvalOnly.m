...
for s = 1:nsub
    ...
    for j = 1:nIter
        ...
        if avgPat
            z = reshape(trainIdx,keepTR,length(trainIdx)/keepTR);
            z = z';
            p = reshape(patterns.regressor.twoCond(1,trainIdx),keepTR,length(trainIdx)/keepTR);
            t = reshape(patterns.regressor.twoCond(2,trainIdx),keepTR,length(trainIdx)/keepTR);
            trainTargs = [p(1,:);t(1,:)];
            clear trainPats
            for i = 1:nTrain
                trainPats(i,:) = mean(patterns.raw_sm_filt_z(z(i,:)+shiftTR,:));
            end
        else
